...
class DistilBertSS2QONNXbase(object):
    ...
    def __init__(self,model_path,provider):
        ...
        self.tokenizer = DistilBertTokenizer.from_pretrained("models/tokenizers/distilbert_ss2")
    ...
    def batchpredict(self, x):
        ...
        return y1[0], y1[1], T, self.colorcombiner( xs, y2 )
    ...
    def colorcombiner(self,senslist,values):
        ...
        for idx, sen in enumerate(senslist):
            if values[idx][1] <= 0.8 and values[idx][1] >= 0.1:
                color = 'yellow'
            elif values[idx][1] <= 0.2:
